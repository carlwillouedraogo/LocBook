...
namespace projetLocBook
{
    public partial class frmReservation : Form
    {
        ...
        static frmReservation instance;
        private static readonly HttpClient client = new HttpClient();

        string urlAchat = "http://127.0.0.1:8000/api/reservation/ajouter";
        private frmReservation()
        {
            InitializeComponent();
        }
            ...
            if (result == DialogResult.Yes)
            {
                try
                {
                    foreach (var ouvrage in ouvrages)
                    {
                        var data = new
                        {
                            date_debut = DateTime.Now.ToString("yyyy-MM-dd"),
                            livre = ouvrage.ouv_id.ToString(),
                            date_fin = dtpDateEmprunt.Value.ToString("yyyy-MM-dd"),
                        };
                        ...
                        {
                            MessageBox.Show("Réservation validé");
                        }
                        else
                        {
        ...
        private void btnAnnuler_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                //si oui affiche la page du panier pour annuler la reservation et ferme la page
                frmPanier panier = new frmPanier();
                panier.Show();
                this.Hide();
            }
        }
        static public frmReservation getInstance()
        {
            if (instance is null || instance.IsDisposed)
            {
                instance = new frmReservation();
            }
            return instance;
        }
    }
